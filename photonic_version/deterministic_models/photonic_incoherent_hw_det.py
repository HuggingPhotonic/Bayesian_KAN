"""
Training script for an incoherent photonic KAN with hardware-style mixers.
"""
...
from photonic_version.bases.hardware import HardwarePhotonicIncoherentBasis
...
class HardwareIncoherentPhotonicKAN(nn.Module):
    def __init__(self, layer_sizes: Sequence[int], *, basis_kwargs: dict):
        ...
        for in_f, out_f in zip(layer_sizes[:-1], layer_sizes[1:]):
            layers.append(HardwarePhotonicIncoherentBasis(in_f, out_f, **basis_kwargs))
...
def train_hardware_incoherent_kan() -> None:
    ...
    model = HardwareIncoherentPhotonicKAN(layer_sizes, basis_kwargs=basis_kwargs).to(device)
    ...
    progress = trange(1, epochs + 1, desc="Training HW Incoherent Photonic KAN", leave=True)
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE loss")
    plt.title("HW Photonic KAN (incoherent) training curve")
