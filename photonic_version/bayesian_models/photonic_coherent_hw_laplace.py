...
def run_laplace_inference() -> None:
    ...
    layer_sizes: Sequence[int] = (1, 32, 32, 1)
    basis_kwargs = {
        "num_rings": 12,
    ...
    mse_loss = nn.MSELoss()
    losses = []
    progress = trange(1, map_epochs + 1, desc="Laplace MAP training", leave=True)
    ...
    with torch.no_grad():
        X_eval_temp = torch.linspace(-1, 1, 800, device=device).unsqueeze(-1)
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE loss")
    plt.title("Laplace MAP training curve")
    ...
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title("Laplace MAP fit (coherent)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
