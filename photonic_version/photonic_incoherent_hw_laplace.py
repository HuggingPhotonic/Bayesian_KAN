"""
Laplace approximation inference for the incoherent hardware photonic KAN.
"""
...
from photonic_version.photonic_incoherent_hw_det import HardwareIncoherentPhotonicKAN
...
def run_laplace_inference() -> None:
    ...
    output_dir = Path(__file__).resolve().parent / "results" / "hardware_incoherent_laplace"
    ...
    plt.title("Laplace MAP optimisation (HW incoherent)")
    ...
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title("Laplace posterior (HW incoherent photonic KAN)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
