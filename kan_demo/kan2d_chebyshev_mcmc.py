"""
Chebyshev polynomial KAN with MCMC (Random-Walk Metropolis & HMC) in 2D.
"""
...
OUTPUT_DIR = Path(__file__).parent / "results_mcmc_chebyshev_2d"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


class ChebyshevLayer(nn.Module):
    def __init__(self, in_features: int, out_features: int, degree: int = 12):
        super().__init__()
...
class ChebyshevKAN(nn.Module):
    def __init__(self, layers_hidden, degree=12):
        super().__init__()
...
def visualise_surfaces(X_grid, Y_grid, Z_true, stats):
    xg = X_grid.cpu()
    yg = Y_grid.cpu()
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "chebyshev_mcmc_surfaces.png", dpi=150, bbox_inches="tight")
...
def visualise_contours(X_grid, Y_grid, Z_true, stats):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "chebyshev_mcmc_contours.png", dpi=150, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Chebyshev KAN 2D MCMC (RWM & HMC)")
    print("=" * 60)
