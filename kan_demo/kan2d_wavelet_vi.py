...
OUTPUT_DIR = Path(__file__).parent / "results_vi_wavelet_2d"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


class WaveletBasis(nn.Module):
    """
    Haar wavelet basis with variational coefficients.
    """

    def __init__(self, in_features: int, out_features: int,
                 levels: int = 4, prior_scale: float = 1.0):
        super().__init__()
    ...
    def _normalize(self, x):
        return (torch.tanh(x) + 1.0) / 2.0  # map to [0, 1]

    def _wavelet_basis(self, x):
        x = self._normalize(x)
        batch, feats = x.shape
        bases = []
    ...
    def forward(self, x, sample=True, n_samples=1):
        ...
        return wave_out + residual, self.kl_divergence()
...
class BayesianWaveletKAN(nn.Module):
    def __init__(self, layers_hidden, levels=4, prior_scale=1.0):
        super().__init__()
        self.layers = nn.ModuleList([
            WaveletBasis(layers_hidden[i], layers_hidden[i + 1],
                         levels=levels, prior_scale=prior_scale)
...
def train_with_variational_inference():
    ...
    model = BayesianWaveletKAN(layers_hidden=[2, 12, 12, 1],
                               levels=4, prior_scale=0.5).to(device)
    ...
    print("\nStart VI training (Wavelet basis)...")
    epochs = 2000
    losses = []

    progress = trange(epochs, desc="Wavelet VI Training", leave=True)
    for epoch in progress:
        model.train()
        optimizer.zero_grad()

        preds, kl = model(X_train, sample=True, n_samples=8)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'wavelet_vi_surfaces.png', dpi=150, bbox_inches='tight')
    print("\nSurface plots saved!")
    ...
    plt.title('Wavelet VI Training Loss')
    plt.yscale('log')
    plt.grid(True, alpha=0.3)
    plt.savefig(OUTPUT_DIR / 'wavelet_vi_loss.png', dpi=150, bbox_inches='tight')
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'wavelet_vi_contours.png', dpi=150, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Wavelet KAN 2D Variational Inference")
    print("=" * 60)
