"""
Wavelet KAN 2D MCMC comparison (Random-Walk Metropolis & HMC).
"""
...
OUTPUT_DIR = Path(__file__).parent / "results_mcmc_wavelet_2d"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


class WaveletLayer(nn.Module):
    def __init__(self, in_features: int, out_features: int, levels: int = 4):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return wave_out + residual


class WaveletKAN(nn.Module):
    def __init__(self, layers_hidden, levels=4):
        super().__init__()
        self.layers = nn.ModuleList([
            WaveletLayer(layers_hidden[i], layers_hidden[i + 1], levels=levels)
...
def train_map(model, x, y, epochs=500, lr=5e-4,
    ...
    losses = []
    progress = trange(epochs, desc="Wavelet MAP 2D", leave=True)
...
def random_walk_metropolis(model, x, y, n_samples=600, burn_in=400,
                           step_size=5e-4, noise_var=0.05, prior_var=1.0):
...
def hmc_sampling(model, x, y, n_samples=600, burn_in=400,
                 step_size=1e-3, n_leapfrog=20,
                 noise_var=0.05, prior_var=1.0):
    theta_map = parameters_to_vector(model.parameters()).detach()
    current = theta_map.clone()
...
def visualise_results(X_grid, Y_grid, Z_true, stats):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "wavelet_mcmc_surfaces.png", dpi=150, bbox_inches="tight")
...
def visualise_contours(X_grid, Y_grid, Z_true, stats):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "wavelet_mcmc_contours.png", dpi=150, bbox_inches="tight")
...
def run_experiment():
    ...
    model = WaveletKAN(layers_hidden=[2, 12, 12, 1], levels=4).to(device)
    ...
    plt.title("Wavelet MAP Training Loss (2D)")
    plt.yscale("log")
    plt.grid(True, alpha=0.3)
    plt.savefig(OUTPUT_DIR / "wavelet_map_loss.png", dpi=150, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Wavelet KAN 2D MCMC (RWM & HMC)")
    print("=" * 60)
